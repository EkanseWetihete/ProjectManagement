...
if [[ "${1:-}" == "--help" ]]; then
  echo "Usage: ./update-and-start-backend-background.sh"
  echo "Fetches and fast-forward pulls the current branch from origin, then starts the backend in the background."
...
exec bash "$SCRIPT_DIR/start-backend-background.sh"